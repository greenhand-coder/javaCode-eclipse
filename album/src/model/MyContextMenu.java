...
public class MyContextMenu {       //按键类  
	 MainUIController mainUI;
	 ContextMenu contextMenu;
	 public MyContextMenu(Node node,MainUIController mainUI,boolean choice) {
		 this.mainUI = mainUI;
		 if(choice) {
			 MenuForPicture(node);     //选中图片时的菜单
		 }
		 MenuForBlank(node);            //没有选中图片时的菜单
		
	}
	public void MenuForPicture(Node node) {
	 ...
	 public void MenuForBlank(Node node) {
		...
		node.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent e) -> {
			...
			if (clickNode instanceof FlowPane && !(clickNode instanceof Thumbnail) && !(clickNode instanceof Text)) {// 鼠标点击非图片节点
				Thumbnail.clearSelected();// 清空已选
