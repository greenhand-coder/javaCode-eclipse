...
//			System.out.println(selectRectangle);
			...
			if(this.isDragged) {				
				...
				for(Node childrenNode:  mainUIController.getFlowPaneChildren()) {
					if(childrenNode instanceof Thumbnail) {
						if(isRectOverlap((Thumbnail)childrenNode))
							((Thumbnail)childrenNode).setSelected(true);
					}
//					((PictureNode)childrenNode).setSelected(false);
	...
	private boolean isRectOverlap(Thumbnail  thumbnail) {
		double imageNodeCenterPointX = thumbnail.getLayoutX() + thumbnail.getWidth()/2.0;
		double imageNodeCenterPointY = thumbnail.getLayoutY() + thumbnail.getHeight()/2.0;
		...
		return Math.abs(imageNodeCenterPointX - selectRectangleCenterPointX) <= (thumbnail.getWidth()/2.0 + selectRectangle.getWidth()/2.0) &&
				Math.abs(imageNodeCenterPointY - selectRectangleCenterPointY) <= (thumbnail.getHeight()/2.0 + selectRectangle.getHeight()/2.0);
	}
