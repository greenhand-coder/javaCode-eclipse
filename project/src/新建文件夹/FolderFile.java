...
public class FolderFile {
	...
	public FolderFile(File imageFile) {
...
//		}
    ...
    public FolderFile(String imagePath) {
    	this(new File(imagePath));
    }
    
    public FolderFile[] listPictures() {
		File[] files = this.imageFile.listFiles();
		if(files == null||files.length == 0) {
			return null;
		}
		FolderFile[] pictureFiles = new FolderFile[files.length];
		for(int i = 0; i<files.length;i++) {
			pictureFiles[i] = new FolderFile(files[i]);
		}
		return pictureFiles;
	}
